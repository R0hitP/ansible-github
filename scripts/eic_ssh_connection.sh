...
output=$(aws ec2-instance-connect send-ssh-public-key --instance-id ${instanceId} --region us-east-1 --instance-os-user ssm-user --ssh-public-key file://~/.ssh/id_rsa.pub --profile UserR)
echo "$output"
done
